...
impl PixState {
    ...
    where
        ...
        S: AsRef<str>,
    {
        self.text_transformed(p, text, None, None, None)
    }
    ...
    /// Frame
    pub fn frame<R, S>(&mut self, rect: R, label: S) -> PixResult<()> where R: Into<Rect<i32> S: AsRef<str> {
        let s = self;
        s.ui.frames.insert(
            0,
            Frame {
    ...
    where
        L: AsRef<str>,
    {
        self._button(label.as_ref(), None)
    }
...
impl PixState {
    fn _text_transformed(
        ...
        Ok(self
            .renderer
            .text(p, text, s.wrap_width, angle, center, flipped, s.fill)?)
    }

    // fn _text_field(&mut self, rect: Rect<i32>, label: &str, value: &mut String) -> PixResult<bool> {
    //     let s = self;
    //     let frame = s.ui.get_current_frame();
    //     if frame.hidden {
    //         return Ok(false);
    //     }

    //     let id = frame.get_id(&label);
    ...
    fn _button(&mut self, label: &str, size: Option<Size>) -> PixResult<bool> {
        let s = self;
        let mut frame = s.ui.get_current_frame();
        if frame.hidden {
            return Ok(false);
        }
        ...
        let (label_w, label_h) = s.size_of(label)?;

        let pos = frame.cursor_pos;
        let size = size.unwrap_or_else(|| (label_w + 2 * pad_x, label_h + 2 * pad_y).into());
        let rect = rect![pos, size.width(), size.height()];
        frame.advance_cursor(size, pad_y);
        s.ui.frames[0] = frame;
    ...
    //             rect.set_x(rect.x() - rect.width() - 2 * pad);
    //         }
    //         if rect.bottom() > h as i32 {
    //             rect.set_y(rect.y() - rect.height() - 2 * pad);
    //         }
    //         s.rect(rect)?;
    //         s.clip(rect)?;
    //         s.fill(s.text_color());
    //         s.text(rect.top_left() + point!(pad, pad), text)?;
    //         s.no_clip()?;
    //     }
